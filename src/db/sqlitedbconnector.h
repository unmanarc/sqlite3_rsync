...
public:
    ...
    std::list<DBQuery *> GetQueriesForOIDS(const std::string &tableName, const std::list<u_int64_t> &oids, u_int64_t * maxoid);
    std::list<DBQuery *> GetQueriesForOIDSGreaterThan(const std::string &tableName,u_int64_t minoid, u_int64_t * lastoid);



private:
    std::list<u_int64_t> GetOIDSGreaterThan(const std::string &tableName, u_int64_t minoid);

    void GetTableComponents(const std::string &tableName, DBQueryConstructor * dbqc);
