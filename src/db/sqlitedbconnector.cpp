...
{
    std::string rsp;
    string xsql = "SELECT sql FROM (SELECT sql sql, type type, tbl_name tbl_name, name name, rowid x FROM sqlite_master UNION ALL SELECT sql, type, tbl_name, name, rowid FROM sqlite_temp_master) WHERE lower(tbl_name) LIKE '?' AND type!='meta' AND sql NOTNULL ORDER BY rowid";
    sqlite3_stmt * stmt;
    sqlite3_prepare_v2(ppDb, xsql.c_str(), xsql.size()+1, & stmt, NULL);
    sqlite3_bind_text(stmt, 1, /* The number of the argument. */
                      table.c_str(),
...
std::list<DBQuery *> SQLiteDBConnector::GetQueriesForOIDS(const std::string &tableName,const std::list<u_int64_t> &oids, u_int64_t *maxoid)
{
    ...
    for (u_int64_t oid : oids)
    {
        ...
        if (maxoid && oid>*maxoid) *maxoid = oid;
    }
    return r;
}
...
void SQLiteDBConnector::FillInsertQuery(DBQueryConstructor *dbqc, u_int64_t oid)
{
    ...
    sqlite3_stmt * stmt;
    sqlite3_prepare_v2(ppDb, ds.query.c_str(), ds.query.size()+1, & stmt, NULL);
    int s;
    s = sqlite3_step (stmt);
    if (s == SQLITE_ROW)
    {
