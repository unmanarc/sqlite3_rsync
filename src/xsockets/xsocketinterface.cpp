...
unsigned int XSocketInterface::ReadUInt32(bool *bok)
{
    unsigned int rsp = 0;
    if (bok) *bok = true;
    if (!Read(&rsp,sizeof(unsigned int)) && bok) *bok=false;
    return htonl(rsp);
}
...
int XSocketInterface::ReadInt32(bool *bok)
{
    int rsp = 0;
    if (bok) *bok = true;
    if (!Read(&rsp,sizeof(int)) && bok) *bok=false;
    return rsp;
}
...
bool XSocketInterface::WriteBlock(const void *data, uint32_t datalen)
{
    ...
    if (!Write(&datalen,sizeof(uint32_t))) return false;
...
bool XSocketInterface::ReadBlock(void *data, uint32_t datalen, bool bestrict)
{
    ...
    if (downSize > datalen)
    {
        ...
        if (ok) memcpy(data,odata,datalen);
        delete [] odata;
        if (!ok) return false;
    }
    else
    {
        ...
        if (!Read(data,downSize)) return false;
    }
    return true;
}
...
void *XSocketInterface::ReadBlockAlloc(unsigned int *datalen)
{
    *datalen = 0;
    uint32_t downSize = 0;
    if (!Read(&downSize,sizeof(uint32_t))) return NULL;
...
{
    if (!WriteUInt32(val.size())) return false;
    for (auto v : val)
    {
        if (!WriteString(v,max)) return false;
    }
    return true;
}
...
bool XSocketInterface::WriteStringLow(const string &val)
{
    ...
    // Enviar
    if (!Write(&upSize,sizeof(u_char))) return false;
    // Enviar datos.
